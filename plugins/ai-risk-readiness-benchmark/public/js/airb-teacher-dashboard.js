/**
 * airb-teacher-dashboard.js
 *
 * Teacher results dashboard — HTML/CSS aligned with archive/demo BenchmarkDashboardsDemo.
 *
 * Depends on: airb-core, airb-results, airb-dashboard-model
 * Exposes: AIRB.TeacherDashboard
 */
'use strict';

(function () {
	window.AIRB = window.AIRB || {};
	var TD = AIRB.TeacherDashboard || {};
	AIRB.TeacherDashboard = TD;
	...
	/**
	 * Demo CoreSummary block for the sticky teacher profile header.
	 */
	TD.coreSummaryHtml = function (model) {
		if (!model) return '';
		...
		html += '</div>';
		html += '<span class="teacher-dash-core__icon" aria-hidden="true">T</span>';
		html += '</div>';
		if (model.motif) {
	...
	function focusStackHtml(model, r, opts) {
		...
		model.focusAreas.forEach(function (area) {
			...
			if (hasGuidance && opts.focusGuidanceAccordionHtml) {
				...
				html += opts.focusGuidanceAccordionHtml('View classroom impact', guidance, false);
			}
			html += '</div>';
		});
	...
	function progressPanelHtml(model, r, opts) {
		var tr = r.teacher_results || {};
		var cert = model.certificate || {};
		var accent = model.accent || '#2563eb';
		var soft = model.soft || '#dbeafe';
		...
		html += '<div><p class="teacher-dash-scene" style="color:' + esc(accent) + '">Progress passport</p>';
		...
		if (tr.next_steps && tr.next_steps.rollout && opts.rolloutSectionHtml) {
			html += opts.rolloutSectionHtml(tr.next_steps.rollout);
		}

		html += (AIRB.Certificate && AIRB.Certificate.panelHtml) ? AIRB.Certificate.panelHtml(model, 'teacher', accent) : '';
		html += '</div>';
		return html;
	}
	...
	TD.render = function (r, model, opts) {
		opts = opts || {};
		if (!model) return '';

		var html = '<section class="teacher-dash-section" data-airb-teacher-dashboard>';
		html += tabsHtml('overview');
		html += '<div class="teacher-dash-panels">';
		html += overviewPanelHtml(model, r, opts);
		html += progressPanelHtml(model, r, opts);
	...
	TD.bind = function (root) {
		if (!root) return;
		var dashboard = root.querySelector('[data-airb-teacher-dashboard]');
		if (!dashboard) return;
