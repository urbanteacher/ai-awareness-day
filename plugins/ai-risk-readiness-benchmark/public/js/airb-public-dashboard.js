...
(function () {
	...
	function peerComparisonHtml(model) {
		...
		var yourColor = peerYourScoreColor(yourScore, averageScore, topQuartile) || model.accent || '#A32D2D';
	...
	function readinessMetricHtml(metric, accent) {
		...
		html += '<p class="teacher-dash-metric__value" style="color:' + esc(accent || '#A32D2D') + '">' + esc(metric.value) + '</p>';
		if (metric.note) {
	...
	function progressPanelHtml(model, opts) {
		...
		var accent = model.accent || '#A32D2D';
