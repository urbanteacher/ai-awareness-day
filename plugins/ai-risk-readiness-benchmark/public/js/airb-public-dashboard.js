...
(function () {
	...
	function focusStackHtml(model, r, opts) {
		...
		var intro = opts.focusStackIntro || 'These cards expand your lowest domain scores into privacy risks and habits to tighten.';
		...
		model.focusAreas.forEach(function (area) {
			...
			if (hasGuidance && opts.focusGuidanceAccordionHtml) {
				...
				html += opts.focusGuidanceAccordionHtml(opts.guidanceToggle || 'View privacy impact', guidance, false);
			}
			html += '</div>';
		});
