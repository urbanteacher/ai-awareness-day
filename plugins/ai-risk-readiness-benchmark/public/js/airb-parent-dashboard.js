/**
 * airb-parent-dashboard.js
 *
 * Parent results dashboard — HTML/CSS aligned with archive/demo BenchmarkDashboardsDemo.
 *
 * Depends on: airb-core, airb-results, airb-dashboard-model
 * Exposes: AIRB.ParentDashboard
 */
'use strict';

(function () {
	window.AIRB = window.AIRB || {};
	var PD = AIRB.ParentDashboard || {};
	AIRB.ParentDashboard = PD;
	...
	PD.coreSummaryHtml = function (model) {
		...
		html += '</div>';
		html += '<span class="teacher-dash-core__icon" aria-hidden="true">P</span>';
		html += '</div>';
		if (model.motif) {
	...
	function strengthCardHtml(model, opts) {
		...
		var heading = (opts.parentResult && opts.parentResult.strengths_heading) || "What you're doing well";
	...
	function progressPanelHtml(model, opts) {
		...
		var accent = model.accent || '#9333ea';
		var soft = model.soft || '#f3e8ff';
	...
	PD.render = function (r, model, opts) {
		...
		var html = '<section class="teacher-dash-section" data-airb-parent-dashboard>';
	...
	PD.bind = function (root) {
		if (!root) return;
		var dashboard = root.querySelector('[data-airb-parent-dashboard]');
		if (!dashboard) return;
