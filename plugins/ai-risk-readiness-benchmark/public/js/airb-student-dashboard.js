/**
 * airb-student-dashboard.js
 *
 * Student results dashboard — HTML/CSS aligned with archive/demo BenchmarkDashboardsDemo.
 *
 * Depends on: airb-core, airb-results, airb-dashboard-model
 * Exposes: AIRB.StudentDashboard
 */
'use strict';

(function () {
	window.AIRB = window.AIRB || {};
	var SD = AIRB.StudentDashboard || {};
	AIRB.StudentDashboard = SD;
	...
	function strengthCardHtml(model, opts) {
		...
		var heading = (opts.studentResult && opts.studentResult.strengths_heading) || "What you're doing well";
	...
	function progressPanelHtml(model, opts) {
		...
		html += (AIRB.Certificate && AIRB.Certificate.panelHtml) ? AIRB.Certificate.panelHtml(model, 'student', accent) : '';
		if (opts.retakeCardHtml) {
			html += '<section class="teacher-dash-card teacher-dash-retake">' + opts.retakeCardHtml() + '</section>';
		}
		html += '</div>';
		return html;
	}
	...
	SD.render = function (r, model, opts) {
		...
		var html = '<section class="teacher-dash-section" data-airb-student-dashboard>';
	...
	SD.bind = function (root) {
		if (!root) return;
		var dashboard = root.querySelector('[data-airb-student-dashboard]');
		if (!dashboard) return;
