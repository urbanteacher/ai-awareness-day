/**
 * airb-leader-dashboard.js
 *
 * School leader results dashboard — HTML/CSS aligned with archive/demo BenchmarkDashboardsDemo.
 *
 * Depends on: airb-core, airb-results, airb-dashboard-model
 * Exposes: AIRB.LeaderDashboard
 */
'use strict';
...
(function () {
	...
	function peerComparisonHtml(model) {
		...
		var yourColor = peerYourScoreColor(yourScore, averageScore, topQuartile) || model.accent || '#475569';

		var html = '<section class="teacher-dash-peer" aria-label="' + esc(peer.comparisonLabel || 'How you compare to similar schools') + '">';
	...
	LD.coreSummaryHtml = function (model) {
		if (!model) return '';
		...
		html += '</div>';
		html += '<span class="teacher-dash-core__icon" aria-hidden="true">L</span>';
		html += '</div>';
		if (model.motif) {
	...
	function strengthCardHtml(model, opts) {
		...
		var heading = (opts.leaderResult && opts.leaderResult.strengths_heading) || 'Strengths include';
	...
	function focusStackHtml(model, r, opts) {
		...
		var intro = opts.focusStackIntro || 'These cards expand the lowest domain scores into governance impact and recommended actions.';
		...
		model.focusAreas.forEach(function (area) {
			...
			if (hasGuidance && opts.focusGuidanceAccordionHtml) {
				...
				html += opts.focusGuidanceAccordionHtml(opts.guidanceToggle || 'View governance impact', guidance, false);
			}
			html += '</div>';
		});
	...
	function progressPanelHtml(model, opts) {
		...
		var accent = model.accent || '#475569';
		var soft = model.soft || '#e2e8f0';
	...
	LD.render = function (r, model, opts) {
		opts = opts || {};
		if (!model) return '';

		var html = '<section class="teacher-dash-section" data-airb-leader-dashboard>';
	...
	LD.bind = function (root) {
		if (!root) return;
		var dashboard = root.querySelector('[data-airb-leader-dashboard]');
		if (!dashboard) return;
