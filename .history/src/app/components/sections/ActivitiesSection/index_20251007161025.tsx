...
const typeToThemeId: Record<string, string> = {
  "lesson-starters": "smart",        // blue
  "tutor-time": "creative",          // green
  "assemblies": "responsible",       // purple
  "after-school-clubs": "future",    // orange
}

function getThemeForType(activityId: string) {
  const themeId = typeToThemeId[activityId]
  return themeConfigs.find((t) => t.id === themeId)
}
              ...
              {activityTypes.map((activity, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div 
                      className={`absolute inset-0 ${getThemeForType(activity.id)?.gradientClass ?? ''} opacity-10 group-hover:opacity-20 transition-opacity duration-300`}
                      style={{
                    ...
                    <div className="relative z-10 mb-4">
                      <div className="flex items-center justify-between mb-3">
                        <div 
                          className={`w-12 h-12 ${getThemeForType(activity.id)?.gradientClass ?? ''} flex items-center justify-center rounded-lg`}
                          style={{
                        ...
                        <span className="text-xs font-bold text-white px-3 py-1 rounded-full bg-purple-600 dark:bg-purple-500">
                          {activity.duration}
                        </span>
                      </div>
                    ...
                    <div className="space-y-2">
                      {activity.features.map((feature, featureIndex) => (
                        <div key={featureIndex} className="flex items-center space-x-2">
                          <div className={`w-2 h-2 ${getThemeForType(activity.id)?.solidClass ?? ''} rounded-full`} />
                    ...
                    >
                      <div className={`w-full h-full ${getThemeForType(activity.id)?.gradientClass ?? ''} rounded-full`} />
                    </div>
                  </div>
                </motion.div>
              ))}
