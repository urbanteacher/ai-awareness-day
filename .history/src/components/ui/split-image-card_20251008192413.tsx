    ...
    >
      ...
      <div className={cn("bg-gradient-to-br", config.gradient, "p-4 sm:p-6 relative overflow-hidden")}>
        ...
        <div className="relative z-10 mb-3 sm:mb-4">
          <div className="w-full h-16 sm:h-20 md:h-24 bg-white/20 rounded-lg overflow-hidden backdrop-blur-sm">
            <img 
              src={imageUrl}
              alt={title}
        ...
        <div className="relative z-10 flex items-start justify-between">
          <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-white leading-tight flex-1 mr-2 sm:mr-4">
            <span className="line-clamp-2">{title}</span>
          </h4>
          <span className={cn("px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-medium rounded-full flex-shrink-0", config.difficultyColor)}>
            {difficultyText}
          </span>
        </div>
      </div>
      ...
      <div className="bg-gray-800 p-4 sm:p-6 flex flex-col">
        ...
        <div className="flex-1">
          <p className="text-white text-sm sm:text-base leading-relaxed line-clamp-3 h-12 sm:h-16 flex items-start">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex flex-wrap gap-1">
              {tags.map((tag, index) => (
                <span 
                  key={index}
                  className="text-xs bg-gray-700 text-gray-300 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded truncate max-w-[80px] sm:max-w-[120px]"
                >
                  {tag}
                </span>
              ))}
