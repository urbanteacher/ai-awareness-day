...
function aiad_register_timeline_meta(): void {
    $meta_fields = array(
        ...
        '_aiad_timeline_likes' => array( 'type' => 'integer', 'default' => 0 ),
    );
    ...
    <article class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" data-entry-id="<?php echo esc_attr( (string) $entry->ID ); ?>">
        ...
        <div class="timeline-entry__body<?php echo $has_media ? ' timeline-entry__body--has-media' : ''; ?>">
            <div class="timeline-entry__text">
                ...
                if ( $timeline_terms && ! is_wp_error( $timeline_terms ) && count( $timeline_terms ) > 1 ) :
                    $term_names = array_map( function ( $t ) { return $t->name; }, $timeline_terms );
                    ?>
                    <p class="timeline-entry__category"><?php echo esc_html( implode( ', ', $term_names ) ); ?></p>
                <?php endif; ?>
            ...
            <?php if ( $has_media ) : ?>
                <div class="timeline-entry__media">
                    <?php if ( $use_lite_yt ) : ?>
                        <?php
                        $yt_thumb = 'https://img.youtube.com/vi/' . esc_attr( $yt_id ) . '/hqdefault.jpg';
                        $yt_title = get_the_title( $entry );
                        ?>
                        <div class="timeline-entry__video-facade timeline-lite-yt" data-video-id="<?php echo esc_attr( $yt_id ); ?>" data-title="<?php echo esc_attr( $yt_title ); ?>" role="button" tabindex="0" aria-label="<?php echo esc_attr( sprintf( __( 'Play video: %s', 'ai-awareness-day' ), $yt_title ) ); ?>">
            ...
            <div class="timeline-entry__actions" aria-label="<?php esc_attr_e( 'Actions', 'ai-awareness-day' ); ?>">
                <button type="button" class="timeline-entry__like" data-entry-id="<?php echo esc_attr( (string) $entry->ID ); ?>" aria-label="<?php esc_attr_e( 'Like this update', 'ai-awareness-day' ); ?>">
...
function aiad_ajax_timeline_like(): void {
    ...
    $count = (int) get_post_meta( $entry_id, '_aiad_timeline_likes', true );
    $count++;
    update_post_meta( $entry_id, '_aiad_timeline_likes', $count );
